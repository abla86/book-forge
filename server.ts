...
function parseSessionCookie(req: Request): string {
  ...
  if (!pair) return "";
  const raw = pair.slice(sessionCookieName.length + 1);
  try {
...
function extractSessionToken(req: Request): string {
  ...
  return parseSessionCookie(req);
}

function resolveSessionUser(req: AuthenticatedRequest): AuthUser {
  if (req.authUser) return req.authUser;
  const token = req.sessionToken || extractSessionToken(req);
  if (!token || token.length < 32) {
    throw new Error("Ugyldig autentiseringstoken.");
  }
...
function getAuthUser(req: Request): AuthUser {
  return resolveSessionUser(req as AuthenticatedRequest);
}
...
function assertString(value: unknown, name: string, maxLength = 10000): string {
  if (typeof value !== "string" || !value.trim()) {
    throw new Error(`${name} er påkrevd.`);
  }
  if (value.length > maxLength) throw new Error(`${name} er for langt.`);
  return value.trim();
}
...
// Protected API routes: identity is derived exclusively from the validated server-side session.
app.use("/api", (req: AuthenticatedRequest, res, next) => {
  ...
  if (publicRoutes.has(routeKey)) return next();
...
app.get("/api/health", (_req, res) => {
  res.json({ status: "ok", service: "bookforge-api", time: new Date().toISOString() });
});
...
app.post("/api/auth/login", (req, res) => {
  ...
  if (loginRateLimited(key)) return res.status(429).json({ error: "For mange innloggingsforsøk. Prøv igjen senere." });
  ...
  res.setHeader("Set-Cookie", `${sessionCookieName}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Strict${secure}; Max-Age=${SESSION_MAX_AGE_SECONDS}`);
...
app.post("/api/auth/logout", (req: AuthenticatedRequest, res) => {
  ...
  const secure = isProduction ? "; Secure" : "";
  res.setHeader("Set-Cookie", `${sessionCookieName}=; Path=/; HttpOnly; SameSite=Strict${secure}; Max-Age=0`);
...
app.get("/api/organizations/:id", (req, res) => {
  ...
  const privileged = user.role === "FOUNDER" || user.role === "ADMIN";
  if (!privileged && user.organizationId !== org.id && user.organizationId !== org.slug) return res.status(403).json({ error: "Adgang nektet." });
  return res.json(org);
});
...
app.get("/api/founder/stats", (req, res) => {
  ...
  return res.json({
    ...
    totalWordsGenerated,
    totalAiCostUsd: CostGuard.getTotalPlatformSpend(),
    totalRevenueUsd,
    activeJobsCount: RateLimiter.getTotalActiveJobs(),
    tokenUsage: CostGuard.getTotalTokens(),
...
app.post("/api/founder/kill-switch", (req, res) => {
  const user = getAuthUser(req);
  try {
    return res.json(EmergencyKillSwitch.setKillSwitch(Boolean(req.body?.active), user, typeof req.body?.reason === "string" ? req.body.reason.slice(0, 500) : undefined));
  } catch (error: unknown) {
    return res.status(403).json({ error: error instanceof Error ? error.message : "Kunne ikke endre Kill Switch." });
  }
});
...
app.get("/api/books", (req, res) => {
  ...
  if (user.role === "FOUNDER" || user.role === "ADMIN") return res.json(all);
  return res.json(all.filter((project) => !project.ownerId || project.ownerId === user.id));
});

app.get("/api/books/:id", (req, res) => {
  const user = getAuthUser(req);
  const book = db.getProject(req.params.id);
  if (!book) return res.status(404).json({ error: "Bokprosjekt ikke funnet." });
  const check = requireProjectAccess(user, book, "read");
  if (!check.allowed) return res.status(403).json({ error: check.reason });
...
app.post("/api/books", (req, res) => {
  ...
  const book: BookProject = {
    ...
    author: typeof data.author === "string" ? data.author.slice(0, 200) : user.name,
    genre: typeof data.genre === "string" ? data.genre.slice(0, 100) : undefined,
    description: typeof data.description === "string" ? data.description.slice(0, 5000) : undefined,
...
app.put("/api/books/:id", (req, res) => {
  ...
  const check = requireProjectAccess(user, existing, "write");
  if (!check.allowed) return res.status(403).json({ error: check.reason });
  const data = req.body ?? {};
  const updated: BookProject = {
    ...existing,
    title: typeof data.title === "string" && data.title.trim() ? data.title.trim().slice(0, 300) : existing.title,
    author: typeof data.author === "string" ? data.author.slice(0, 200) : existing.author,
    genre: typeof data.genre === "string" ? data.genre.slice(0, 100) : existing.genre,
    description: typeof data.description === "string" ? data.description.slice(0, 5000) : existing.description,
    chapters: Array.isArray(data.chapters) ? data.chapters : existing.chapters,
    settings: data.settings && typeof data.settings === "object" ? data.settings : existing.settings,
    id: existing.id,
...
app.delete("/api/books/:id", (req, res) => {
  ...
  const existing = db.getProject(req.params.id);
  if (!existing) return res.status(404).json({ error: "Bokprosjekt ikke funnet." });
  const check = requireProjectAccess(user, existing, "admin");
  if (!check.allowed) return res.status(403).json({ error: check.reason });
...
app.get("/api/books/:id/bible", (req, res) => {
  ...
  const check = requireProjectAccess(user, project, "read");
  if (!check.allowed) return res.status(403).json({ error: check.reason });
  const stored = db.getBible(req.params.id);
  return res.json(stored || new BibleEngine(project).getData());
});
...
app.put("/api/books/:id/bible", (req, res) => {
  ...
  const project = db.getProject(req.params.id);
  if (!project) return res.status(404).json({ error: "Bokprosjekt ikke funnet." });
  const check = requireProjectAccess(user, project, "write");
  if (!check.allowed) return res.status(403).json({ error: check.reason });
  ...
  AuditLogger.log({ actorId: user.id, actorRole: user.role, action: "UPDATE_PROJECT", projectId: req.params.id, status: "SUCCESS", metadata: { target: "BOKBIBEL" } });
...
app.post("/api/book/generate-full-book", async (req, res) => {
  ...
  if (!project) {
    const now = new Date().toISOString();
    project = {
      id: body.projectId ? String(body.projectId).slice(0, 100) : `proj-${crypto.randomUUID()}`,
      ownerId: user.id,
      title: title || "Uten tittel",
      idea,
      genre: typeof body.genre === "string" ? body.genre.slice(0, 100) : "Roman",
      tone: typeof body.tone === "string" ? body.tone.slice(0, 100) : "Realistisk",
      lengthLabel: "Standard",
      targetWords: Number.isFinite(Number(body.targetWords)) ? Math.max(1000, Math.min(300000, Number(body.targetWords))) : 80000,
      currentWords: 0,
      targetChapters: Number.isFinite(Number(body.targetChapters)) ? Math.max(1, Math.min(200, Number(body.targetChapters))) : 32,
      synopsis: "",
      coverStyle: "Minimalistisk",
      phase: "planned",
      progress: 0,
      activeChapter: 1,
      acts: Number.isFinite(Number(body.acts)) ? Math.max(1, Math.min(8, Number(body.acts))) : 4,
      pov: typeof body.pov === "string" ? body.pov.slice(0, 100) : "Tredjeperson personlig",
      ending: "Lukket",
      author: typeof body.author === "string" ? body.author.slice(0, 200) : user.name,
      chapters: [], characters: [], locations: [], timeline: [], continuityRules: [], covers: [],
  ...
  try {
    const job = await FullBookEngine.startGeneration({
      ...
      author: typeof body.author === "string" ? body.author.slice(0, 200) : project.author || user.name,
      genre: typeof body.genre === "string" ? body.genre.slice(0, 100) : project.genre,
      subgenre: typeof body.subgenre === "string" ? body.subgenre.slice(0, 100) : undefined,
      tone: typeof body.tone === "string" ? body.tone.slice(0, 100) : project.tone,
      audience: typeof body.audience === "string" ? body.audience.slice(0, 200) : undefined,
      language: typeof body.language === "string" ? body.language.slice(0, 100) : "Norsk (Bokmål)",
      pov: typeof body.pov === "string" ? body.pov.slice(0, 100) : project.pov,
      targetWords: Number(body.targetWords) || project.targetWords || 80000,
      targetChapters: Number(body.targetChapters) || project.targetChapters || 32,
      acts: Number(body.acts) || project.acts || 4,
      user,
    }, ai);
    ...
    return res.status(500).json({ error: error instanceof Error ? error.message : "Kunne ikke starte bokgenerering." });
  }
});
...
app.post("/api/book/generation/:jobId/resume", async (req, res) => {
  ...
  try {
    return res.json(await FullBookEngine.resumeJob(req.params.jobId, ai, user));
  } catch (error: unknown) {
    return res.status(500).json({ error: error instanceof Error ? error.message : "Kunne ikke gjenoppta genereringsjobb." });
  }
});
...
app.post("/api/assets/generate", async (req, res) => {
  ...
  if (!check.allowed) return res.status(403).json({ error: check.reason });
  try {
    const asset = await AssetEngine.getInstance().generateAsset(project, {
      projectId: project.id,
      type: String(body.type).slice(0, 50),
      title: String(body.title).slice(0, 300),
      customPrompt: typeof body.customPrompt === "string" ? body.customPrompt.slice(0, 5000) : undefined,
      chapterNumber: Number.isFinite(Number(body.chapterNumber)) ? Number(body.chapterNumber) : undefined,
      characterName: typeof body.characterName === "string" ? body.characterName.slice(0, 200) : undefined,
      aspectRatio: typeof body.aspectRatio === "string" ? body.aspectRatio.slice(0, 30) : undefined,
    ...
    return res.status(500).json({ error: error instanceof Error ? error.message : "Kunne ikke generere visuelt element." });
  }
});
...
app.post("/api/book/generate-synopsis", async (req, res) => {
  ...
  try {
    ...
    return res.status(error instanceof Error && error.message.includes("budget") ? 402 : 500).json({ error: error instanceof Error ? error.message : "Kunne ikke generere synopsis." });
...
app.post("/api/book/write-chapter", async (req, res) => {
  ...
  if (!check.allowed) return res.status(403).json({ error: check.reason });
  try { EmergencyKillSwitch.assertCanGenerate(); } catch (error: unknown) { return res.status(503).json({ error: error instanceof Error ? error.message : "AI er stanset.", killSwitchActive: true }); }
  const jobId = `job-chapter-${String(body.chapterNumber || "x")}-${crypto.randomUUID()}`;
  ...
  try {
    ...
    const bibleEngine = new BibleEngine(body.bibleData && typeof body.bibleData === "object" ? body.bibleData : { title: body.bookTitle || project.title, genre: body.genre || project.genre, tone: body.tone || project.tone });
    const bibleContext = bibleEngine.buildContextForChapter(chapterNumber, body.characters);
    const prompt = `Du er en skjønnlitterær forfatter på toppnivå. Skriv et komplett, velskrevet kapittel ${chapterNumber} i romanen "${String(body.bookTitle || project.title).slice(0, 300)}".\nSjanger: ${String(body.genre || project.genre || "Roman").slice(0, 100)}\nTone: ${String(body.tone || project.tone || "Realistisk").slice(0, 100)}\nKapitteltittel: ${String(body.chapterTitle || `Kapittel ${chapterNumber}`).slice(0, 300)}\nHandling/Mål: ${String(body.chapterSummary || "").slice(0, 10000)}\nForrige kapittel: ${String(body.previousSummary || "Romanens åpning").slice(0, 10000)}\nRelevante karakterer: ${String(body.characters || "Hovedpersonen").slice(0, 10000)}\n\n${bibleContext}\n\nSkriv levende, sanselig norsk med naturlig dialog, atmosfære, spenning og kontinuitet. Start direkte med kapittelets åpningssetning.`;
    const response = await ai.models.generateContent({ model: AI_MODEL, contents: prompt });
    ...
    return res.status(error instanceof Error && error.message.toLowerCase().includes("budget") ? 402 : 500).json({ error: error instanceof Error ? error.message : "Kunne ikke skrive kapittel." });
  } finally { RateLimiter.releaseJobSlot(user.id, jobId); }
});
...
app.post("/api/book/generate-character-journey", async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: error instanceof Error ? error.message : "Kunne ikke generere karakterreise." });
  }
});
...
app.post("/api/book/deep-continuity-audit", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (parsed && typeof parsed.score === "number") result = { ...baseReport, ...parsed, analysisType: "ai_assisted" };
    } catch { /* keep deterministic rule-based report */ }
    ...
    return res.status(500).json({ error: error instanceof Error ? error.message : "Feil ved kontinuitetsanalyse." });
  }
});
...
app.post("/api/book/versions/revert-chapter", (req, res) => {
  ...
  try {
    const result = VersionService.revertChapter(projectId, body.currentProject, Number(body.chapterNumber), Number(body.targetVersionNumber), user.id);
...
app.use((_req, res) => res.status(404).json({ error: "Ressursen finnes ikke." }));
app.use((error: unknown, _req: Request, res: Response, _next: NextFunction) => {
  console.error("[BookForge] Unhandled request error:", error);
...
startServer().catch((error) => {
  console.error("[BookForge] Server startup failed:", error);
  process.exitCode = 1;
});
